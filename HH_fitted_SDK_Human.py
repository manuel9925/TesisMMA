...
# Creando instancia para llamada a API
...
respuesta = sweep_data["response"][0:index_range[1]+1] # en V
...
sampling_rate = sweep_data["sampling_rate"] # en Hz
...
#    V, m, h, n = y  # State variables
#    g_Na, g_K, g_L, E_Na, E_K, E_L, C_m = params  # Parameters to fit
...
# Simulando con parametros genericos
synthetic_data = solve_ivp(hh_model, [0, float(np.max(t))], y0, args=(I_ext,C_m,g_Na,g_K,g_L,V_Na,V_K,V_L), t_eval=t, method='RK45')

V_synthetic = synthetic_data['y'][0]  

V_observed= respuesta

# Grafica la data sintetica
plt.plot(t, V_synthetic, label='respuesta')
...
def loss_function(params, experimental_data, time_points):
    g_Na, g_K, g_L, V_Na, V_K, V_L, C_m = params
    I_ext=17.0

    # Corre la simulacion con los parametros dados
    sim_data = odeint(hh_model_wrapper, y0, time_points, args=(I_ext,C_m,g_Na,g_K,g_L,V_Na,V_K,V_L))
    #sim_data = solve_ivp(hh_model, 
    #                     [0, float(np.max(time_points))], 
    #                     y0, 
    #                     args=(I_ext,C_m,g_Na,g_K,g_L,V_Na,V_K,V_L), 
    #                     t_eval=time_points, 
    #                     method='RK45')
...
#result2 = minimize(loss_function, optimized_params, args=(V_observed, t),tol=1e-6, method='Powell',options={'maxiter': 1000, 'disp': True})

#optimized_params2 = result2.x

#result3 = minimize(loss_function, optimized_params2, args=(V_observed, t),tol=1e-6, method='Nelder-Mead',options={'maxiter': 1000, 'disp': True})
...
fitted_data = solve_ivp(hh_model, [0, float(np.max(t))], 
                        y0, 
                        args=(I_ext,optimized_params[6],
                              optimized_params[0],optimized_params[1],
                              optimized_params[2],optimized_params[3],
                              optimized_params[4],optimized_params[5]), 
                        t_eval=t, 
                        method='RK45')

V_fitted = fitted_data['y'][0]  # fitted voltage trace

# Plot the synthetic data vs. fitted data
plt.plot(t, V_observed, label='Data Observada', color='blue')
#plt.plot(t, V_synthetic, label='Synthetic Data', color='red', linestyle='dashed')
plt.plot(t, V_fitted, label='Modelo Ajustado', color='green', linestyle='dashed')
#plt.title('Synthetic vs Fitted Membrane Potential')
plt.xlabel('Tiempo (ms)')
plt.ylabel('Potencial de Membrana (mV)')
plt.legend()
plt.show()
